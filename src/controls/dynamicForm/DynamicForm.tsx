...
> {
  ...
  constructor(props: IDynamicFormProps) {
    ...
    this._formulaEvaluation = new FormulaEvaluation(
      this.props.context,
      this.props.webAbsoluteUrl,
    );
      ...
      this.setState(
        ...
        () => {
          ...
          if (
            ...
            prevProps.listItemId !== this.props.listItemId
          ) {
            this.getListInformation()
              .then(() => {
                /* no-op; */
  ...
  public render(): JSX.Element {
    ...
    if (!customFormattingDisabled && customFormatting?.header) {
      headerContent = this._customFormatter.renderCustomFormatContent(
        customFormatting.header,
        this.getFormValuesForValidation(),
        true,
      ) as JSX.Element;
    }
    ...
    if (!customFormattingDisabled && customFormatting?.body) {
      ...
      if (bodySections.length > 0) {
        ...
        const omittedFields = fieldCollection
          .filter((f) => !specifiedFields.includes(f.label))
          .map((f) => f.label);
    ...
    if (!customFormattingDisabled && customFormatting?.footer) {
      footerContent = this._customFormatter.renderCustomFormatContent(
    ...
    return (
      <div>
        {infoErrorMessages.map((ie, i) => (
          <MessageBar key={i} messageBarType={ie.type}>
            {ie.message}
          </MessageBar>
        ))}
          ...
          <div>
            ...
            {bodySections.length > 0 &&
              ...
              bodySections
                .filter(
                  (bs) =>
                    bs.fields.filter((bsf) => hiddenByFormula.indexOf(bsf) < 0)
                      .length > 0,
                )
                .map((section, i) => (
                  <>
            ...
            {(bodySections.length === 0 || customFormattingDisabled) &&
              fieldCollection.map((f, i) => this.renderField(f))}
            {footerContent}
            {!this.props.disabled && (
              <Stack
              ...
              >
                <PrimaryButton
                  disabled={isSaving}
                  text={this.props.saveButtonLabel || strings.Save}
                  onClick={() => this.onSubmitClick()}
                />
                <DefaultButton
                  disabled={isSaving}
                  text={this.props.cancelButtonLabel || strings.Cancel}
  ...
  private renderField = (field: IDynamicFieldProps): JSX.Element => {
    ...
    if (
      ...
      return fieldOverrides[field.columnInternalName]({
        ...field,
  ...
  private onSubmitClick = async (): Promise<void> => {
    ...
    /* Triggers any validation to be shown */
    this.setState({
    ...
    try {
      ...
      fields.forEach((field) => {
        ...
        if (
          ...
          if (
            field.newValue < field.minimumValue ||
      ...
      if (!validationDisabled) {
        validationErrors = (await this.evaluateFormulas(
          this.state.validationFormulas,
          true,
          true,
          this.state.hiddenByFormula,
      ...
      for (let i = 0, len = fields.length; i < len; i++) {
        ...
        if (
          fieldcolumnInternalName.startsWith("_x") ||
      ...
      if (listItemId) {
        try {
          ...
          if (onSubmitted) {
            onSubmitted(
              iur.data,
              returnListItemInstanceOnSubmit !== false ? iur.item : undefined,
            );
          }
        } catch (error) {
  ...
  private addFileToLibrary = async (objects: {}): Promise<void> => {
    ...
    if (selectedFile !== undefined) {
      try {
        ...
        const fileCreatedResult = await library.rootFolder.files.addChunked(
          encodeURI(itemTitle),
          await selectedFile.downloadFileContent(),
        );
  ...
  private onChange = async (
    ...
    if (field.fieldType === "Lookup" || field.fieldType === "LookupMulti") {
      field.stringValue = newValue
        .map((nv) => nv.key + ";#" + nv.name)
        .join(";#");
    }
    if (
      field.fieldType === "TaxonomyFieldType" ||
  ...
  private performValidation = (skipFieldValueValidation?: boolean): void => {
    ...
    if (
      Object.keys(clientValidationFormulas).length ||
      ...
      this.setState(
        ...
        () => {
          const clientSideValidationDisabled =
            useClientSideValidation === false;
          const fieldValidationDisabled = useFieldValidation === false;
          const hiddenByFormula: string[] = !clientSideValidationDisabled
            ? this.evaluateColumnVisibilityFormulas()
            : [];
          let validationErrors = { ...this.state.validationErrors };
          if (!skipFieldValueValidation && !fieldValidationDisabled)
            validationErrors = this.evaluateFieldValueFormulas(hiddenByFormula);
  ...
  private evaluateColumnVisibilityFormulas = (): string[] => {
    return this.evaluateFormulas(
      this.state.clientValidationFormulas,
      false,
    ) as string[];
  };
  ...
  private evaluateFormulas = (
    ...
    for (let i = 0; i < Object.keys(formulas).length; i++) {
      ...
      if (formulas[fieldName]) {
        const field = fieldCollection.find(
          (f) => f.columnInternalName === fieldName,
        );
        if (!field) continue;
  ...
  private getListInformation = async (): Promise<void> => {
    ...
    try {
      ...
      const listInfo = await this._spService.getListFormRenderInfo(
        listId,
        this.webURL,
      );
      ...
      const additionalInfo =
        await this._spService.getAdditionalListFormFieldInfo(
          listId,
          this.webURL,
        );
      const numberFields = additionalInfo.filter(
        (f) => f.TypeAsString === "Number" || f.TypeAsString === "Currency",
      );

      // Build a dictionary of validation formulas and messages
      const validationFormulas: Record<
        string,
        Pick<ISPField, "ValidationFormula" | "ValidationMessage">
      > = additionalInfo.reduce((prev, cur) => {
      ...
      const contentTypeName: string =
        listInfo.ContentTypeIdToNameMap[contentTypeId];
      ...
      const clientValidationFormulas = listInfo.ClientForms.Edit[
        contentTypeName
      ].reduce(
        (prev, cur) => {
      ...
      if (listItemId !== undefined && listItemId !== null && listItemId !== 0) {
        item = await spList.items
          .getById(listItemId)
          .get()
          .catch((err) =>
            this.updateFormMessages(MessageBarType.error, err.message),
          );
      ...
      if (tempFields.filter((f) => f.fieldType === "Currency").length > 0) {
        installedLanguages =
          await sp.web.regionalSettings.getInstalledLanguages();
      }
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private async buildFieldCollection(
    listInfo: IRenderListDataAsStreamClientFormResult,
    contentTypeName: string,
    item: any,
    numberFields: ISPField[],
    listId: string,
    listItemId: number,
    ...
    for (
      let i = 0, len = listInfo.ClientForms.Edit[contentTypeName].length;
      i < len;
      i++
    ) {
      ...
      if (hiddenFields.indexOf(field.InternalName) < 0) {
        ...
        if (field.FieldType === "Number" || field.FieldType === "Currency") {
          const numberField = numberFields.find(
            (f) => f.InternalName === field.InternalName,
          );
          if (numberField) {
        ...
        if (field.FieldType === "User") {
          if (item !== null) {
            ...
            value = userEmails;
            stringValue = userEmails
              ?.map((dv) => dv.split("/").shift())
              .join(";");
            if (item[field.InternalName + "Id"]) {
              subPropertyValues.id = item[field.InternalName + "Id"];
            }
            subPropertyValues.title = userEmails?.map((dv) =>
              dv.split("/").pop(),
            )[0];
        ...
        if (field.FieldType === "DateTime") {
          ...
          defaultDayOfWeek = (
            await this._spService.getRegionalWebSettings(this.webURL)
          ).FirstDayOfWeek;
        }
  ...
  private cultureNameLookup(lcid: number): string {
    ...
    return (
      this.state.installedLanguages?.find((lang) => lang.Lcid === lcid)
        .DisplayName ?? pageCulture
    );
  }
  ...
  private renderFileSelectionControl = (): React.ReactElement => {
    const { selectedFile, missingSelectedFile } = this.state;

    const labelEl = (
      <label className={styles.fieldRequired + " " + styles.fieldLabel}>
        {strings.DynamicFormChooseFileLabel}
      </label>
    );
    ...
    return (
      <div>
        ...
        <FilePicker
          ...
          accepts={
            this.props.supportedFileExtensions
              ? this.props.supportedFileExtensions
